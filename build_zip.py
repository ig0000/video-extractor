...
manifest_code = r"""{
  ...
  "name": "通用视频信息提取器",
  "version": "2.2",
  "description": "通过官方 API 提取 YouTube 视频元数据，支持 B 站",
  "permissions": [
    "activeTab",
    "scripting"
  ],
...
popup_html_code = r"""<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <style>
    body { width: 350px; padding: 15px; font-family: system-ui, sans-serif; }
    button { 
      width: 100%; padding: 10px; margin-bottom: 15px; 
      background: #0078D7; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px;
    }
    button:hover { background: #005A9E; }
    #result { 
      white-space: pre-wrap; font-size: 13px; background: #2d2d2d; color: #ccc;
      padding: 12px; border-radius: 6px; display: none; word-break: break-all;
      font-family: 'Courier New', Courier, monospace;
    }
  </style>
</head>
<body>
  <button id="extractBtn">提取并复制 Markdown</button>
  <div id="result"></div>
...
files = {
    ...
    "popup.html": popup_html_code,
    "popup.js": popup_js_code
}

zip_filename = "video-extractor-final.zip"
with zipfile.ZipFile(zip_filename, 'w') as zipf:
    for filename, content in files.items():
        zipf.writestr(filename, content)

print(f"打包完成！已在当前目录下生成压缩包：{zip_filename}")
